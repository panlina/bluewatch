...
static duk_ret_t js_Client_send(duk_context *ctx) {
	...
	auto len = duk_get_int(ctx, 1);
	auto timeout = (TickType_t)duk_get_uint(ctx, 2);
	auto n = esp_websocket_client_send_text(handle, data, len, timeout);
	duk_push_int(ctx, n);
	return 1;
}
...
void push_Client_prototype(duk_context *ctx) {
	...
	duk_push_c_function(ctx, js_Client_send, 3);
